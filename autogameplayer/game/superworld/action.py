...
class SuperWorldAction(UnitActionBase):
    ...
    def template_path(self, mode, name):
        dir = r"E:\project\AutoGamePlayer\data\template\superWorld"
        return os.path.join(dir, self.platform, mode, f"{name}.png")
    ...
    def init_app(self):
        ...
        botton_bar_click_h = int(self.window_h - (self.botton_bar_h/2))
        ...
        self.str2pos = {
            ...
            "diamond2coin": (bar_unit_w7*7, int(self.window_h - (self.botton_bar_h*2))), 
            ...
            "ever_garden": (bar_unit_w7*9,  int(self.window_h - (self.botton_bar_h*5))), 
            "garden_reward": (bar_unit_w6*11,  int(self.window_h - (self.botton_bar_h*2.5))), 
            "cljy": (380, 530), 
            "explore_reward": (bar_unit_w7*2, int(self.window_h - (self.botton_bar_h*2))),
            "fast_explore": (bar_unit_w6*11, int(self.window_h - (self.botton_bar_h*2))), 
            "explore": (bar_unit_w6*6, int(self.window_h - (self.botton_bar_h*2))), 
            "back2hall": (bar_unit_w6*2, botton_bar_click_h), 
            "raffle": (bar_unit_w6*6, int(self.window_h - (self.botton_bar_h*1.5))), # 抽奖
            "attack": (bar_unit_w6*3, int(self.window_h - (self.botton_bar_h*3))), 
            "gift": (70, 525), 
            "get_gift1": (210, 500), 
            "get_gift2": (550, 870), 
            "friend": (bar_unit_w7*13, int(self.window_h - (self.botton_bar_h*4.5))), 
            "give_receice_heart": (bar_unit_w7*11, int(self.window_h - (self.botton_bar_h*1))), 

            # == wild ==
            "wild": (bar_unit_w7 * 3,  botton_bar_click_h),
            "trial": (bar_unit_w7 * 3,  int(self.window_h - (self.botton_bar_h*2.5))), 
            "alliance": (bar_unit_w7 * 5,  botton_bar_click_h), 

            # == adventure ==
            "adventure": (bar_unit_w7 * 7,  botton_bar_click_h), 
            "reward": (bar_unit_w7 * 13, int(self.window_h - (self.botton_bar_h*1.5))),
            "fight1": (bar_unit_w7 * 7, int(self.window_h - (self.botton_bar_h*1.5))), 
            "fight2": (bar_unit_w7 * 7, int(self.window_h - (self.botton_bar_h*1.5))), 
            ...
            "chat_alliance": (bar_unit_w7*8, int(self.window_h - (self.botton_bar_h*2)))
        }
    ...
    def daily_routine(
        ...
        # while get_loc_on_screen(self.template_path("adventure", "adventure"), 
        #                             self.window_rect, confidence=0.9) is not None:
        #     # 判断是否回到对应界面，不然就一直点，避免打到首领卡住
        #     self.logger.info("未返回冒险界面")
        #     self.seq_click(["skip", "default"])
        #     random_sleep(10, 15)
    ...
    def daily_routine_first(self):
        ...
        self.seq_click(["gift"])
        self.seq_click(["pos6_3", "get_gift1", "get_gift2", "pos6_3", 
    ...
    def ever_garden(self, reward=False, playground=False, hole=False):
        ...
        if playground:
            self.seq_click(['pos6_6'])
            self.seq_click(['raffle'], interval=1.0)

            while get_loc_on_screen(self.template_path("maincity", "subscribe_privilege"), 
                                    self.window_rect, confidence=0.9) is None:
                # check stop, 是否出现订阅特权
                print("抽奖未结束~")

                is_attack = get_loc_on_screen(self.template_path("maincity", "select_attack"), 
                                    self.window_rect, confidence=0.9) is not None
                # check attack, 是否进入攻击状态
                if is_attack:
                    # TODO: 之后遇到双弓的时候再进行修补， 已完成三弓的情况
                    # 攻击的话会有龙的长时间的攻击动画
                    self.seq_click(['attack'], min_short_wait=12, max_short_wait=15)
            ...
            self.seq_click(['default', 'pos6_1'])
        self.seq_click(['pos6_1'])
    ...
    def trial(self, loop=1, direct=False, skip_right=False, mode=None, return_main=True):
        ...
        if mode is not None:
            # 17-20之后，需要选择对应洞窟
            pos = get_loc_on_screen(self.template_path("wild", mode), 
                                    self.window_rect, confidence=0.9)
            self.click(pos)
    ...
    def alliance_help(self):
        ...
        pos_list = get_all_loc_on_screen(self.template_path("chat", "help_green"), 
                                    self.window_rect, confidence=0.9)
